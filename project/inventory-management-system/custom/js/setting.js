$(document).ready(function() {
	...
	// change bio
	$("#changeBioForm").unbind('submit').bind('submit', function() {
		var form = $(this);

		var bio = $("#bio").val();

		if(bio == "") {
			$("#bio").after('<p class="text-danger">Bio field is required</p>');
			$("#bio").closest('.form-group').addClass('has-error');
		} else {

			$(".text-danger").remove();
			$('.form-group').removeClass('has-error');

			$("#changeBioBtn").button('loading');
			...
			$.ajax({
				...
				success:function(response) {
					...
					if(response.success == true)  {												
						...
						// shows a successful message after operation
						$('.changeBioMessages').html('<div class="alert alert-success">'+
	            '<button type="button" class="close" data-dismiss="alert">&times;</button>'+
	            '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> '+ response.messages +
	          '</div>');
	          ...
	          $(".alert-success").delay(500).show(10, function() {
						...
						// shows a successful message after operation
						$('.changeBioMessages').html('<div class="alert alert-warning">'+
	            '<button type="button" class="close" data-dismiss="alert">&times;</button>'+
	            '<strong><i class="glyphicon glyphicon-exclamation-sign"></i></strong> '+ response.messages +
	          '</div>');
